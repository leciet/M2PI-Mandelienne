...
```{r}
origin_sorensen <- dist.binary(new_full,method = 5) #Long 
...
matrice_ochiai <- as.data.frame(as.matrix(origin_ochiai))
o_ochiai <- matrice_ochiai[6126:7089,1:6125]
o_ochiai_mat <- as.matrix(o_ochiai)
```
...
asso_acm_eucli <- as.matrix(asso_acm_eucli)
asso_acm_manhattan <- as.matrix(asso_acm_manhattan)
asso_origin_ochiai <- as.matrix(asso_origin_ochiai)
asso_origin_sorensen <- as.matrix(asso_origin_sorensen)
#Test poids egaux 
poids <- c(0.25, 0.25, 0.25,0.25)
matrices <- list(asso_acm_eucli, asso_acm_manhattan, asso_origin_ochiai, asso_origin_sorensen)
...
mat_consensus_var <- Reduce(`+`, Map(`*`, matrices, poids))
mat_consen
```
...
res.list <- AssignGene(as.matrix(mat_consensus_var), method = "seuil", 0.51)
View(res.list[[1]])
```
