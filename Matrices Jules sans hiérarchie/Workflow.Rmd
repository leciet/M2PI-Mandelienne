...
::: {#matrice_creuse style="test-align:justify"}
...
L'ensemble de ces distances peuvent être pertinentes selon le contexte. Dans notre cas, nous privilégierons les calculs de distances ne prenant pas en compte les absences communes. 
...
Le transport optimal est une méthode qui permet de déterminer la manière la plus efficace de redistribuer une masse (ou une probabilité) d'un point à un autre, tout en minimisant un coût global associé au déplacement, souvent basé sur une mesure de distance ou un critère spécifique. Dans notre cas, on cherche à déplacer les maladies simples sur les maladies complexes.
:::
...
L'objectif de cette section est d'obtenir en output les coordonnées de nos maladies dans un espace commun. Cependant, nos méthodes actuelles ne permettent pas d'obtenir directement ce résultat. En effet, si l'ACM et l'embedding transforment notre matrice de présence/absence en une matrice de coordonnées ce n’est pas le cas des méthodes classiques de calcul de distances qui génèrent une matrice de distances. Il est nécessaire d'appliquer une technique de MDS unfolding, que nous expliquerons ci-dessous, pour obtenir un résultat de nature similaire.

Ici nous choisissons arbitrairement de représenter nos maladies dans **384 dimensions** car il s'agit du nombre de dimensions que l'on peut obtenir avec l'embedding (facteur limitant).
:::

## MDS unfolding

::: {#intro_mds style="text-align:justify"}
La **MDS unfolding** (Multidimensional Scaling unfolding) est une méthode de réduction dimensionnelle permettant de représenter deux ensembles distincts, comme des individus et leurs préférences, dans un même espace euclidien. Contrairement à la MDS classique qui traite des matrices symétriques, elle est adaptée aux matrices dissymétriques (rectangulaires), où les relations sont définies entre deux groupes différents. L’objectif est de projeter ces ensembles dans un espace de faible dimension tout en respectant au mieux les distances ou proximités initiales. Les individus et les objets sont représentés par des points, et la distance entre eux reflète leur relation observée dans les données. Cette méthode est particulièrement utile pour visualiser et interpréter les préférences ou associations entre deux ensembles dans des contextes complexes.
:::
...
L'objectif de cette ACM (Analyse des Correspondances Multiples) est de projeter les maladies complexes dans un espace défini par les maladies simples. Pour ce faire, les maladies complexes sont traitées comme des individus supplémentaires, tandis que l'espace de représentation est construit exclusivement à partir des maladies simples.
:::
...
::: {#intro_permut style="text-align:justify"}
Une fois les coordonnées obtenues, il est nécessaire de définir un seuil permettant d’apparier les maladies complexes aux gènes. Autrement dit, il s'agit de déterminer une valeur critique au-delà de laquelle une association entre une maladie complexe et une maladie simple sera considérée comme significative.

Dans cette section, nous expliquons comment la p-value critique a été estimée pour chaque maladie. L’objectif est d’évaluer les associations significatives en permutant aléatoirement les valeurs binaires (0 et 1) associées à la maladie complexe, tout en maintenant la proportion initiale de 0 et 1 et en conservant inchangées les données des autres maladies simples. Ces permutations brisent les relations existantes, et après chaque permutation, les distances entre la maladie complexe et les maladies simples sont recalculées. En répétant ce processus plusieurs fois, on construit une distribution empirique des distances sous l’hypothèse nulle (absence d’association réelle). La valeur minimale des distances obtenues pour chaque permutation est utilisée comme seuil critique. Enfin, la distance réelle entre la maladie complexe non permutée et les maladies simples est comparée à cette distribution : si elle est inférieure au seuil critique, l’association est jugée significative, confirmant qu’elle n’est pas due au hasard. Ce processus permet d’identifier des relations pertinentes tout en contrôlant le risque d’erreur statistique.
:::
...
La réalisation d'une classification pose certaines contraintes. Habituellement, un individu sera présent dans une seule classe. Or dans notre contexte, nous considérons qu'un gène est en mesure d'expliquer plusieurs maladies complexes.
:::
...
:::{#intro_llm style="text-align:justify"}

Une fois les matrices d'assignation obtenues, nous pouvons récupérer une liste de gènes pour chaque maladie complexe. Afin d'interpréter ces listes, nous faisons appel à un large modèle de langage (LLM).

L'objectif est de donner, à chaque association gène-maladie, un score de pertinence prenant en compte la littérature sur le sujet, les processus biologiques impactés et la comparaison avec des gènes et des processus similaires.

Pour des raisons d'automatisation, nous nous sommes concentrés sur les llm gratuits. Nous avons utilisé `llama3` issu de Ollama pour obtenir un score d'association avec le prompt suivant.

*Question: Based on the function of the `[GENE]` gene, could this gene be involved in the development of `[MALADIE]`? Quantify on a scale from 0 to 10 the potential causal relationship between `[GENE]` variations and `[MALADIE]`, considering both direct evidence and mechanistic plausibility.*
...
Nous réalisons, pour chaque association gène-maladie, plusieurs itérations de ce prompt. Un score moyen et un intervalle de confiance sont ensuite calculés. 

Le temps de calcul pour l'ensemble des associations étant très long, nous avons créé une application Shiny afin de limiter les calculs nécessaires et permettre à l'utilisateur de regarder uniquement les associations d'intérêt pour lui. Cette application récupère uniquement les différentes matrices d'associations dans le fichier `association_matrices_2.RData` et permet de créer une matrice consensus. 
...
load('association_matrix_sorensen.RData')
sorensen_permut <- association_matrix
load('association_matrix_sokal_sneath.RData')
sokal_sneath_permut <- association_matrix_sokal_sneath
load('association_matrix_sokal_michener.RData')
sokal_michener_permut <- association_matrix
load('association_matrix_rogers_tanimoto.RData')
rogers_tanimoto_permut <- association_matrix
load('association_matrix_ochiai.RData')
ochiai_permut <- association_matrix
load('association_matrix_jaccard.RData')
jaccard_permut <- association_matrix
load('association_matrix_hamann.RData')
hamann_permut <- association_matrix
load('association_matrix_cosine.RData')
cosine_permut <- association_matrix_cosine

load("assignment_matrix_sorensen_classif.RData")
sorensen_classif <- assignment_matrix_sorensen_classif
load("assignment_matrix_ochiai_classif.RData")
ochiai_classif <- assignment_matrix_ochiai_classif
load("assignment_matrix_jaccard_classif.RData")
jaccard_classif <- assignment_matrix_jaccard_classif
load("assignment_matrix_ot_jaccard_classif.RData")
ot_jaccard_classif <- assignment_matrix_ot_jaccard_classif
load("assignment_matrix_embedding_classif.RData")
embedding_classif <- assignment_matrix_embedding_classif
load("assignment_matrix_acm_classif.RData")
acm_classif <- assignment_matrix_acm_classif
load("assignment_matrix_sokal_michener_classif.RData")
sokal_michener_classif <- assignment_matrix_sokal_michener_classif
load('assignment_matrix_sokal_sneath_classif.RData')
sokal_sneath_classif <- assignment_matrix_sokal_sneath_classif
load("assignment_matrix_rogers_tanimoto_classif.RData")
rogers_tanimoto_classif <- assignment_matrix_rogers_tanimoto_classif
load("assignment_matrix_hamann_classif.RData")
hamann_classif <- assignment_matrix_hamann_classif
load("assignment_matrix_cosine_classif.RData")
cosine_classif <- assignment_matrix_cosine_classif


association_matrices <- list(jaccard_permut=jaccard_permut,
                             jaccard_classif=jaccard_classif,
                             ot_jaccard_classif=ot_jaccard_classif,
                             sorensen_permut=sorensen_permut,
                             sorensen_classif=sorensen_classif,
                             sokal_sneath_permut=sokal_sneath_permut,
                             sokal_sneath_classif=sokal_sneath_classif,
                             ochiai_permut=ochiai_permut,
                             ochiai_classif=ochiai_classif,
                             cosine_permut=cosine_permut,
                             cosine_classif=cosine_classif,
                             rogers_tanimoto_permut=rogers_tanimoto_permut,
                             rogers_tanimoto_classif=rogers_tanimoto_classif,
                             hamann_permut=hamann_permut,
                             hamann_classif=hamann_classif,
                             sokal_michener_permut=sokal_michener_permut,
                             sokal_michener_classif=sokal_michener_classif,
                             acm_classif=acm_classif,
                             embedding_classif=embedding_classif
                             )

save(association_matrices,file='association_matrices_2.RData')
