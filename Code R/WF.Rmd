...
## Etape 2 : Association des maladies entrse elles à partir des distances, obtention d’un pool de matrices d’assignation
...
```{r}
matrices <- list(as.matrix(asso_acm_eucli), as.matrix(asso_acm_manhattan), as.matrix(asso_origin_ochiai), as.matrix(asso_origin_sorensen))
...
mat_consensus<- Reduce(`+`, Map(`*`, matrices, poids))
...
res.list <- AssignGene(mat_consensus, method = "seuil", 0.7)
View(res.list[[1]])
